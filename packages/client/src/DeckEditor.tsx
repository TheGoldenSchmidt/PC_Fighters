...
import { deckProblems, newDeckId, setCardCount, type SavedDeck } from './deckLibrary';
...
export function DeckEditor({ faction, factions, cards, rules, initial, initialSub, onSave, onCancel }: Props) {
  ...
  const allowed = useMemo(() => cards.filter((c) => (parent[c.faction] ?? c.faction) === faction), [cards, faction, parent]);
  ...
  return <section className="panel deck-editor">
    ...
    <div className="deck-curve"><strong>{total}/{rules.size}</strong>{curve.map((n, i) => <span key={i}>⚡{i === 5 ? '5+' : i}: {n}</span>)}</div>
    ...
    <div className="deck-card-list">{visible.map((card) => {
      const count = countOf(card.id); const max = card.signature ? (rules.maxCopiesSignature ?? 1) : rules.maxCopies;
      return <article key={card.id} className="deck-builder-card">
        ...
        <div><strong>{card.name}{card.signature ? ' ★' : ''}</strong><small>⚡ {card.cost} · {factions.find((f) => f.id === card.faction)?.name}</small><p>{card.text}</p></div>
        <div className="copy-stepper"><button aria-label={`${card.name} entfernen`} disabled={count === 0} onClick={() => setEntries(setCardCount(entries, card.id, count - 1))}>−</button><b>{count}</b><button aria-label={`${card.name} hinzufügen`} disabled={count >= max || total >= rules.size} onClick={() => setEntries(setCardCount(entries, card.id, count + 1))}>+</button></div>
      </article>;
    })}</div>
