...
**Designer kopiert** das passende Fragment und **tunt nur die Zahlen**
...
Warum die Ketten so gebaut sind, steht in `PLAYBOOK.md` → „Rig-Rezepte"; woran das
Ergebnis gemessen wird, in `QUALITY_CRITERIA.md`.
...
**hier ergänzen/korrigieren** (nicht duplizieren) – analog zu `PLAYBOOK.md` und
`QUALITY_CRITERIA.md`.
...
`visual.height` für mittelgroße Tiere ~0.6 (siehe `QUALITY_CRITERIA.md` → Linse A).
...
unsichtbar auf dem Nachbarn – siehe `QUALITY_CRITERIA.md` → Linse B; Kontrast heißt
Helligkeitssprung, nicht nur andere Farbe).
